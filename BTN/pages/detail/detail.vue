			...
			<view v-else>
				...
				<view class="like">
					<view class="btn" :class="detailObj.isLike ? 'active' : '' " @click="clickLike">
						<u-icon class="btntb" name="thumb-up-fill" color="#ffffff" size="30"></u-icon>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 获取评论列表
			async getComment(){
				let commentTemp=db.collection("luntan_comment").where(`article_id == "${this.artid}" && comment_type==0`).orderBy("comment_date desc").limit(20).getTemp();
				...
				let replyArr=await db.collection("luntan_comment").where({
			...
			// 获取部分点赞的用户
			getLikeUser() {
				let likeTemp = db.collection("luntan_like").where(`article_id== "${this.artid}"`).getTemp()
			...
			// 修改阅读量
			readUpData() {
				utilsObj.operation("luntan_article", "view_count", this.artid, 1).then(res => {
					console.log(res);
				})
			},
			...
			// 获取数据库数据
			getData() {
				let artTemp = db.collection("luntan_article").where(`_id=="${this.artid}"`).getTemp();
				let userTemp = db.collection("uni-id-users").field("_id,username,nickname,avatar_file").getTemp();



				let likeTemp = db.collection("luntan_like").where(`article_id=="${this.artid}" && user_id==$cloudEnv_uid`)
					.getTemp();
				...
				db.collection(...tempArr).get({
					...
					let isLike = false;
					if (store.hasLogin) isLike = res.result.data._id.luntan_like.length ? true : false;
